...
- **Поръчки** — преглед и управление на статуси; клиентските поръчки от кошницата пристигат тук (към кухнята)
...
- Активна кошница: добавяне на артикули и изпращане на поръчка към кухнята
...
2. Деплойни backend-а (`boot.js`) в Railway и задай env `DATABASE_URL=mysql://...` и `ADMIN_TOKEN=<дълъг случаен низ>`.
3. Впиши Railway URL-а в `config.js` (по подразбиране работи same-domain през Vercel rewrite).
...
**Env за admin панела:** `ADMIN_TOKEN` — admin панелът иска този таен низ при вход и го праща в header `x-admin-token`; backend-ът го сравнява с env `ADMIN_TOKEN`. Задай силна стойност в Railway.
